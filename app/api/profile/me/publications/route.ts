...
export async function GET() {
  ...
  return NextResponse.json({ success: true, data: profile.publications });
}
...
export async function POST(req: Request) {
  ...
  const d = parsed.data;
  const maxOrder = profile.publications.length ? Math.max(...profile.publications.map((p) => p.order)) : -1;
