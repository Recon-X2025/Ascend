...
export async function GET() {
  ...
  return NextResponse.json({ success: true, data: profile.projects });
}
...
export async function POST(req: Request) {
  ...
  const d = parsed.data;
  const maxOrder = profile.projects.length ? Math.max(...profile.projects.map((p) => p.order)) : -1;
  const created = await prisma.project.create({
    data: {
      ...
      associatedWith: d.associatedWith ?? undefined,
      order: maxOrder + 1,
    },
  });
