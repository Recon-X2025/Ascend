...
export async function POST(req: Request) {
  try {
    ...
    if ((session.user as { role?: string }).role !== "JOB_SEEKER") {
      return NextResponse.json({ error: "Only job seekers can generate cover letters" }, { status: 403 });
    }
    ...
    return NextResponse.json({ error: "Failed to queue cover letter" }, { status: 500 });
  }
}
