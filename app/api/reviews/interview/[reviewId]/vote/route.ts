...
export async function POST(
  ...
  const review = await prisma.interviewReview.findUnique({
    where: { id: reviewId },
    select: { id: true, userId: true, helpfulCount: true, unhelpfulCount: true },
  });
  if (!review) {
  ...
  const existing = await prisma.interviewVote.findUnique({
  ...
  await prisma.$transaction([
    prisma.interviewVote.upsert({
      where: { reviewId_userId: { reviewId, userId } },
      create: { reviewId, userId, helpful },
      update: { helpful },
    }),
    prisma.interviewReview.update({
      where: { id: reviewId },
      data: {
        ...(deltaHelpful !== 0 && { helpfulCount: { increment: deltaHelpful } }),
        ...(deltaUnhelpful !== 0 && { unhelpfulCount: { increment: deltaUnhelpful } }),
      },
    }),
  ]);

  const updated = await prisma.interviewReview.findUnique({
    where: { id: reviewId },
  ...
  return NextResponse.json({
    ...
    unhelpfulCount: updated?.unhelpfulCount ?? review.unhelpfulCount,
  });
}
