...
export async function POST(req: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  if (ct.includes("multipart/form-data")) {
    ...
    if (type !== "PHOTO" || !file || !(file instanceof File))
      return NextResponse.json({ success: false, error: "type PHOTO and file required" }, { status: 400 });
    const photoCount = await prisma.companyMedia.count({ where: { companyId: company.id, type: "PHOTO" } });
    if (photoCount >= 10) return NextResponse.json({ success: false, error: "Max 10 photos" }, { status: 400 });
    if (file.size > MAX) return NextResponse.json({ success: false, error: "File max 5MB" }, { status: 400 });
    if (!ALLOWED.includes(file.type)) return NextResponse.json({ success: false, error: "Only JPG, PNG, WebP" }, { status: 400 });
  ...
  if (!parsed.success) return NextResponse.json({ success: false, error: "type and url required" }, { status: 400 });
  const existing = await prisma.companyMedia.count({ where: { companyId: company.id, type: parsed.data.type } });
  if (existing >= 1) return NextResponse.json({ success: false, error: "Max 1 " + parsed.data.type.toLowerCase() }, { status: 400 });
  const maxOrder = await prisma.companyMedia.findFirst({ where: { companyId: company.id }, orderBy: { order: "desc" }, select: { order: true } }).then((r) => (r ? r.order : -1));
