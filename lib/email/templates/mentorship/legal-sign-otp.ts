...
export async function sendLegalSignOTP(params: {
  to: string;
  otp: string;
  documentTitle: string;
}) {
  if (!resend) return;
  const { to, otp, documentTitle } = params;
  await resend.emails.send({
    from: FROM,
    to: [to],
    subject: `Your signing code — ${APP_NAME}`,
    html: `
<!DOCTYPE html>
<html>
...
<body style="font-family:system-ui,-apple-system,sans-serif;line-height:1.6;color:#333;max-width:480px;margin:0 auto;padding:24px;">
  ...
  <p>Use this code to sign <strong>${documentTitle}</strong>. It expires in 10 minutes.</p>
