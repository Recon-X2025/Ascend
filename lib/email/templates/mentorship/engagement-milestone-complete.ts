...
export async function sendMilestoneComplete(params: {
  mentorEmail: string;
  menteeEmail: string;
  mentorName: string;
  menteeName: string;
  ...
  const client = resend;
  if (!client) return;
  const { mentorEmail, menteeEmail, mentorName, menteeName, milestoneType, engagementUrl } = params;
  const sendOne = async (to: string, name: string) => {
    await client!.emails.send({
      from: FROM,
      to: [to],
      subject: `Milestone complete — ${APP_NAME}`,
      html: `
<!DOCTYPE html>
<html>
...
<body style="font-family:system-ui,-apple-system,sans-serif;line-height:1.6;color:#333;max-width:480px;margin:0 auto;padding:24px;">
  ...
  <h1 style="font-size:20px;color:#16A34A;">${milestoneType} milestone complete</h1>
  <p>Hi ${name},</p>
  <p>Both parties have filed assessments for the ${milestoneType} milestone.</p>
