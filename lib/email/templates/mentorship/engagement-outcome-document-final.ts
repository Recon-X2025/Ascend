...
export async function sendOutcomeDocumentFinal(params: {
  mentorEmail: string;
  menteeEmail: string;
  mentorName: string;
  menteeName: string;
  ...
  const sendOne = async (to: string, name: string) => {
    await client!.emails.send({
      from: FROM,
      to: [to],
      subject: `Outcome document finalised — ${APP_NAME}`,
      html: `
<!DOCTYPE html>
<html>
...
<body style="font-family:system-ui,-apple-system,sans-serif;line-height:1.6;color:#333;max-width:480px;margin:0 auto;padding:24px;">
  ...
  <h1 style="font-size:20px;color:#16A34A;">Outcome document finalised</h1>
  <p>Hi ${name},</p>
  <p>Both parties have signed the outcome document. Your engagement is formally complete.</p>
